...
class Locator:
    def __init__(self, timeStamp, mismatchSignals, bindDicts, terms, parameters, moduleInfo) -> None:
        self.mismatchSignals = mismatchSignals
    ...
    def dynamicSlice(self, tmpInputSignalValues, tmpOracleSignalValue):
        for mismatchSignal in self.mismatchSignals:
            spsBindDicts, executedPaths, availableTerms, availableParamters, availableRenameInfo = [], [], [], [], []
            tmpInputSignalValue = tmpInputSignalValues[mismatchSignal]
            index = mismatchSignal.index('.')
            instance, signal = mismatchSignal[:index], mismatchSignal[index+1:]
            module = self.moduleInfo[instance]
            sTerms, sParameters = self.filterTermsAndParameters(module, signal)
            sRenameInfo = {}
            subInstance = mismatchSignal[:mismatchSignal.rindex('.')]
            ...
            for bd in bindDict:
                ...
                inputSignalValue = tmpInputSignalValue[mismatchSignal]
                inputSignalValue = inputSignalValue[inputSignalValue.index("'b")+2:]
                oracleSignalValue = tmpOracleSignalValue[mismatchSignal]
                ...
                if lsb == None and msb == None and ptr == None:
                    ...
                    if SignalAnalyzer.equal(sv, oracleSignalValue): continue
                else:
                    try:
                        if ptr != None:
                            ptr = int(ptr)
                            if sv == None: sv = inputSignalValue[::-1][ptr]
                            if SignalAnalyzer.equal(sv, oracleSignalValue[::-1][ptr]): continue
                        else:
                            lsb, msb = int(lsb), int(msb)
                            if sv == None: sv = inputSignalValue[::-1][lsb:msb+1]
                            if SignalAnalyzer.equal(sv, oracleSignalValue[::-1][lsb:msb+1]): continue
                    except:
                        print("Out of Bound!")
                        continue
